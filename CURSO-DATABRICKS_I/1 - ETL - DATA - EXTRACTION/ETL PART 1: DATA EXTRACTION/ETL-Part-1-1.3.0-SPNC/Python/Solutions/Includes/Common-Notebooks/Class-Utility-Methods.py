...
def assertDbrVersion(expMajor:int, expMinor:int):
  
  (major, minor) = getDbrMajorAndMinorVersions()

  if (major < expMajor) or (major == expMajor and minor < expMinor):
    msg = f"This notebook must be ran on DBR {expMajor}.{expMinor} or better, found DBR {major}.{minor}. You must update your cluster configuration before proceeding."
    raise AssertionError(msg)
    
  if major > expMajor:
    displayHTML(f"""<span style="color:red; font-weight:bold">WARNING: This notebook must be ran on DBR {expMajor}.x, found DBR {major}.{minor}.</span>""")
    displayHTML(f"""
      ...
      <div style="font-weight:bold">Using an untested DBR may yield unexpected results and random errors.</div>
    ...
    displayHTML(f"Running on <b>DBR {major}.{minor}</b>")
...
def getDatabaseName(courseType:str, username:str, moduleName:str, lessonName:str) -> str:
  import re
  langType = "p" # for python
  databaseName = username + "_" + moduleName + "_" + lessonName + "_db" + langType + courseType
...
def classroomCleanup(daLogger:object, courseType:str, username:str, moduleName:str, lessonName:str, dropDatabase:str): 
  ...
  for stream in spark.streams.active:
    ...
    actions += f"""<li>The stream "{stream.name}" has been terminated.</li>"""
    print(         f"""The stream "{stream.name}" has been terminated.""")
  ...
  try:
    ...
    for row in tables:
      ...
      actions += f"""<li>The table "{tableName}" has been dropped.</li>"""
      print(         f"""The table "{tableName}" has been dropped.""")
  ...
  if dropDatabase: 
    ...
    actions += f"""<li>The database "{database}" has been dropped.</li>"""
  ...
  if pathExists(path):
    ...
    actions += f"""<li>Cleaned out our working directory: {path}</li>"""
    print(         f"""Cleaned out our working directory: {path}""")
    
  htmlMsg = "Cleaning up the learning environment..."
  if len(actions) == 0:
    actions += "<li>No actions taken.</li>"
...
.multiple_choicex.checkedRed {
  ...
  <body>
    ...
    <p>Please let us know if you liked this lesson, <b>LESSON_NAME_UNENCODED</b></p>
