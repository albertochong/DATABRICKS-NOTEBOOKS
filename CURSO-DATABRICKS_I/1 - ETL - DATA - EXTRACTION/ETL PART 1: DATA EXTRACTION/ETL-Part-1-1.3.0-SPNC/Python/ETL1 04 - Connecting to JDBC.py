...
dfMin = peopleDF.select(col("id")).min()
dfMax = peopleDF.select(col("id")).max()
...
# MAGIC %md
# MAGIC ## JDBC Connections : SQL SERVER
...
try:
  jdbcHostname = "srvchong.cj7ywu7i2hm9.us-east-2.rds.amazonaws.com"
  jdbcDatabase = "dbLisbonMetropolitan"
  jdbcPort = 1433
  jdbcUrl = "jdbc:sqlserver://{0}:{1};database={2};user={3};password={4}".format(jdbcHostname, jdbcPort, jdbcDatabase, "achong", "1qaz2wsX")

  tableName = "Tbl_BusStops"
  #query = "(select * from Tbl_BusStops where idBusStop = 2) emp_alias"
  
  #busStopDF = spark.read.jdbc(url=jdbcUrl, table=tableName)
  
  #-------------------------------Criando partiçoes para executar a query
  busStopDF = spark.read.jdbc(url=jdbcUrl, table=tableName, lowerBound=1, upperBound=100000, numPartitions=100)
