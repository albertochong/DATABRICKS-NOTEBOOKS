...
def classroomCleanup(daLogger:DatabricksAcademyLogger, courseType:String, username:String, moduleName:String, lessonName:String, dropDatabase: Boolean):Unit = {
  ...
  for (stream <- spark.streams.active) {
    ...
    val action = s"""The stream "${stream.name}" has been terminated."""
    actions += action
    println(action)
  }
  ...
  try {
    ...
    for (row <- tables){
      ...
      val action = s"""The table "${tableName}" has been dropped."""
      actions += action
      println(action)
    } 
  } catch {
  ...
  if (dropDatabase){
    ...
    val action = s"""The database "${database}" has been dropped."""
    actions += action
    println(action)
  }
  ...
  if (pathExists(path)) {
    ...
    val action = s"""Cleaned out our working directory: $path"""
    actions += action
    println(action)
  }
