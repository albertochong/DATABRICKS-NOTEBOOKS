...
def assertDbrVersion(expMajor:Int, expMinor:Int):String = {
  ...
  if ((major < expMajor) || (major == expMajor && minor < expMinor)) {
    throw new AssertionError(s"This notebook must be ran on DBR $expMajor.$expMinor or better, found DBR $major.$minor. You must update your cluster configuration before proceeding.")
  }
  if (major != expMajor || minor < expMinor) {
    ...
    displayHTML(s"Running on <b>DBR $major.$minor</b>")
  }
...
def getDatabaseName(courseType:String, username:String, moduleName:String, lessonName:String):String = {
  val langType = "s" // for scala
  var databaseName = username + "_" + moduleName + "_" + lessonName + "_db" + langType + courseType
...
def classroomCleanup(daLogger:DatabricksAcademyLogger, courseType:String, username:String, moduleName:String, lessonName:String, dropDatabase: Boolean):Unit = {
  ...
  for (stream <- spark.streams.active) {
    ...
    actions += s"""<li>The stream "${stream.name}" has been terminated.</li>"""
    println(       s"""The stream "${stream.name}" has been terminated.""")
  }
  ...
  try {
    ...
    for (row <- tables){
      ...
      actions += s"""<li>The table "${tableName}" has been dropped.</li>"""
      println(       s"""The table "${tableName}" has been dropped.""")
    } 
  } catch {
  ...
  if (dropDatabase){
    ...
    actions += s"""<li>The database "${database}" has been dropped.</li>"""
    println(       s"""The database "${database}" has been dropped.""")
  }
  ...
  if (pathExists(path)) {
    ...
    actions += s"""<li>Cleaned out our working directory: $path</li>"""
    println(       s"""Cleaned out our working directory: $path""")
  }
